$(function() {	
	
	$("#btnStoreBookmarks").on("click", function(){
	
		var memberId = $("#memberId").val();
		var storeNo = $("#storeNo").val();
	
		var d = $(this).attr("class");

		if( d.includes('fa-heart-o')){
			location.href="http://localhost:8080/yameokja/bookmarksAdd?memberId=" + memberId + "&storeNo=" + storeNo;
			
			$(this).attr("class", "fa fa-heart bookmarks-on text-danger");
			alert("true");

			return false;
			
		}else{
			location.href="http://localhost:8080/yameokja/bookmarksDelete?memberId=" + memberId + "&storeNo=" + storeNo;
			
			$(this).attr("class", "fa fa-heart-o bookmarks-off");
			alert("false");
			
			return false;
		}	
	});
	
	$(document).ready(function(){
	
		alert("열림!");
		
		var memberBookmarks = $("#memberBookmarks").val();
		
		var d = $("#btnStoreBookmarks").attr("class");

		if( d.includes('fa-heart-o')){
			$(this).attr("class", "fa fa-heart bookmarks-on text-danger");
			alert("true");

			return false;
			
		}else{
			$(this).attr("class", "fa fa-heart-o bookmarks-off");
			alert("false");
			
			return false;
		}	
	})

});